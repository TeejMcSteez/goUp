...
import type { MQTTConfig, StatusMessage as StatusMsg } from "../../types";
...
export default function MQTTPanel({ mqtt, onRefresh }: MQTTPanelProps) {
  ...
  const set = (field: keyof MQTTFormData) => (e: React.ChangeEvent<HTMLInputElement>) =>
      ...
      <form className="flex flex-col gap-4 p-4 bg-elevated border border-border rounded-lg" onSubmit={handleSave}>
        <div className="grid grid-cols-[repeat(auto-fit,minmax(220px,1fr))] gap-4">
          <label className="flex flex-col gap-1 text-[0.85rem] font-medium text-muted">
            Broker URL
            <input className={inputClass} value={form.Mqtt_broker} onChange={set("Mqtt_broker")} placeholder="mqtt://broker.example.com" />
          </label>
          <label className="flex flex-col gap-1 text-[0.85rem] font-medium text-muted">
            Username
            <input className={inputClass} value={form.Mqtt_username} onChange={set("Mqtt_username")} placeholder="username" />
          </label>
          <label className="flex flex-col gap-1 text-[0.85rem] font-medium text-muted">
            Key / Password
            <input className={inputClass} type="password" value={form.Mqtt_key} onChange={set("Mqtt_key")} placeholder="••••••••" />
          </label>
        </div>
