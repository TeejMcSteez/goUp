...
const cols = ["Name", "URL", "Description", "API URL", "API Key", "Valid Responses", "Retries", "Skip TLS Verify"] as const;
...
export default function BulkEditPanel({ services, onSave, onCancel }: BulkEditPanelProps) {
  const [rows, setRows] = useState<RowData[]>(() =>
    Object.entries(services).map(([key, svc]) => ({
      ...
      SkipInsecure: svc.SkipInsecure ?? false,
    }))
  );
  ...
  const toggleSkipInsecure = (idx: number) => () =>
    setRows((prev) => prev.map((r, i) => (i === idx ? { ...r, SkipInsecure: !r.SkipInsecure } : r)));
  ...
  return (
    <div className="flex flex-col gap-4 p-3 sm:p-4 bg-elevated border border-border rounded-lg">
      ...
      <div className="hidden sm:grid sm:grid-cols-[1fr_2fr_2fr_2fr_2fr_1.5fr_0.6fr_0.9fr] gap-2 text-muted text-[0.8rem] font-medium pb-1 border-b border-border">
      ...
      <div className="flex flex-col gap-3">
        {rows.map((row, idx) => (
          <div
            key={row.key}
            className="flex flex-col gap-2 p-3 bg-surface border border-border rounded-lg sm:p-0 sm:bg-transparent sm:border-x-0 sm:border-t-0 sm:rounded-none sm:grid sm:grid-cols-[1fr_2fr_2fr_2fr_2fr_1.5fr_0.6fr_0.9fr] sm:gap-2 sm:items-center sm:pb-2"
          >
            ...
            <div className="flex flex-col gap-1">
              {row.API_URL && (
                <>
                  <label className="sm:hidden text-muted text-[0.75rem] font-medium">API Key</label>
                  <input className={inputClass} type="password" value={row.API_Key} onChange={setCell(idx, "API_Key")} placeholder="API key" />
                </>
              )}
            </div>
            <div className="flex flex-col gap-1">
            ...
            <div className="flex flex-col gap-1">
              <label className="sm:hidden text-muted text-[0.75rem] font-medium">Skip TLS Verify</label>
              <input
                type="checkbox"
                className="cursor-pointer justify-self-start"
                checked={row.SkipInsecure}
                onChange={toggleSkipInsecure(idx)}
              />
            </div>
          </div>
        ))}
